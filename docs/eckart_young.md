...
Let $A \in \mathbb{R}^{m \times n}$, $r = \operatorname{rank}(A)$. The singular value decomposition of $A$ is
...
$$\|A\|_F^2 = \sum_{i=1}^{r} \sigma_i^2. \tag{1}$$

For $1 \le k < r$ define the truncation $A_k = \sum_{i=1}^{k} \sigma_i u_i v_i^{\top}$, so that $\operatorname{rank}(A_k) = k$ and $A - A_k = \sum_{i>k} \sigma_i u_i v_i^{\top}$.

## 2. Statement

**Theorem (Eckart–Young 1936; Mirsky 1960).** For every $B \in \mathbb{R}^{m \times n}$ with $\operatorname{rank}(B) \le k$ and every unitarily invariant norm $\|\cdot\|$,

$$\|A - B\| \;\ge\; \|A - A_k\|.$$

In particular,

$$\min_{\operatorname{rank}(B) \le k} \|A - B\|_2 = \sigma_{k+1}, \qquad \min_{\operatorname{rank}(B) \le k} \|A - B\|_F = \Bigl(\sum_{i=k+1}^{r} \sigma_i^2\Bigr)^{1/2}. \tag{2}$$

The minimizer is unique iff $\sigma_k > \sigma_{k+1}$. Note that $\{B : \operatorname{rank}(B) \le k\}$ is non-convex, yet the global minimum is attained in closed form.

## 3. Proof

**Spectral norm.** Let $\operatorname{rank}(B) \le k$. Then $\dim \ker(B) \ge n - k$ and $\dim \operatorname{span}(v_1, \dots, v_{k+1}) = k+1$; since $(n-k) + (k+1) > n$, there exists a unit vector $w = \sum_{i=1}^{k+1} c_i v_i \in \ker(B)$, $\sum c_i^2 = 1$. Hence
...
$$\sigma_i(A - B) \ge \sigma_{i+k}(A) \quad \text{for all } i \ge 1. \tag{3}$$

Summing squares gives $\|A - B\|_F^2 \ge \sum_{i > k} \sigma_i^2(A) = \|A - A_k\|_F^2$. $\blacksquare$

**General unitarily invariant norms (Mirsky).** Inequality (3) shows that the vector $\bigl(\sigma_i(A - B)\bigr)_i$ weakly majorizes the tail $\bigl(\sigma_{k+i}(A)\bigr)_i$. By von Neumann's theorem, every unitarily invariant norm is a symmetric gauge function $\Phi$ of the singular values, and every such $\Phi$ is monotone under weak majorization. Therefore $\|A - B\| = \Phi\bigl(\sigma(A-B)\bigr) \ge \Phi\bigl(\sigma(A - A_k)\bigr) = \|A - A_k\|$. $\blacksquare$
...
$$\frac{\|A - A_k\|_F^2}{\|A\|_F^2} = \frac{\sum_{i>k} \sigma_i^2}{\sum_{i \le r} \sigma_i^2}. \tag{4}$$
